...
```mermaid
graph TD
    A[U.S. Constitution] --> B[Article XXVIII: Guard Pact]
    B --> C[All Laws Must Pass 5 HLGs]
    C --> D[Fail Any = Void]
    C --> E[Pass All = Enforced]
    E --> F[Sunset + Circuit-Breakers]
    E --> G[State Labs for 13 Issues]
    F --> H[Auto-Correction]
    G --> I[Data Wins]    
```
